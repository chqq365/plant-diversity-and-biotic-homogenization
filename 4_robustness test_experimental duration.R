...
for(yr in c(10, 14)){ 
  for(i in c(0)){
    for(fg in c("all",  "FORB", "GRAMINOID",  "LEGUME",  "WOODY", "NAT", "INT")){
      # yr<-14; i<-0; fg<-"all"
      load(paste0("model for alpha diversity for ", fg , " species with q of ", i, " for sites with ", yr, "-year nutrient addition.Rdata"))
      #plot(mod.alpha)
      #pp_check(mod.alpha)
      load(paste0("model for gamma diversity for ", fg , " species with q of ", i, " for sites with ", yr, "-year nutrient addition.Rdata"))
      #plot(mod.gamma)
      #pp_check(mod.gamma)
      ...
      alpha.gamma.intercepts.individual.sites0<-alpha.diversity_coef2%>%mutate(scale="alpha")%>%
        bind_rows(gamma.diversity_coef2%>%mutate(scale="gamma"))%>%mutate(q=i, functional_group=fg, experimental.year=yr)
      ...
      alpha.gamma.intercepts0<-alpha.diversity_fixef%>%as.data.frame()%>%mutate(terms=rownames(.), scale="alpha")%>%
        ...
        mutate(q=i, functional_group=fg, experimental.year=yr)
      ...
      beta.across.sites0 <- bind_cols(gather_draws(mod.alpha, `b_trtNPK`) %>%
        ...
        mutate(scale="beta", terms="", q=i, functional_group=fg, experimental.year=yr)%>%select(scale, terms, Estimate, Q2.5,  Q97.5, q, functional_group, experimental.year)
      ...
      diversity.individual.sites00<-diversity.individual.sites0%>%mutate(q=i, functional_group=fg, experimental.year=yr)  
...
# write.csv(categorize.beta, file="estimated alpha, gamma, and beta diversity for at individual sites with longer term.csv")

###########################################################################################
################################ make more customized figures #############################
###########################################################################################

summary.intercepts.across.sites<-summary.intercepts.across.sites%>%mutate(combi=paste0(functional_group, "_", experimental.year,"_", q))
categorize.beta<-categorize.beta%>%mutate(combi=paste0(functional_group, "_", experimental.year,"_", q))
source("function to draw figures.R")

# for all species at year 10 and 14
for(ey in c(10, 14)){
  i<-0; fg<-"all"; sc<-"beta"; 
  summary.intercepts.across.sites.w<-summary.intercepts.across.sites%>%filter(terms =="trtNPK")%>%filter(scale!=sc)%>%filter(q==i & functional_group==fg & experimental.year==ey)%>%
    pivot_wider(names_from = "scale", values_from = c("Estimate", "Q2.5", "Q97.5"))
  overall.change.in.beta<- summary.intercepts.across.sites%>%filter(q==i & functional_group==fg & scale==sc & experimental.year==ey)%>%
    mutate(dif.positive=Q97.5 - Estimate, dif.negative=Estimate - Q2.5)
  # site-level data 
  summary.intercepts.individual.sites_i_fg<-categorize.beta%>%filter(q==i & functional_group==fg & experimental.year==ey)
  ...
  ggsave(pp.all, width = 18.4, height = 18.4, dpi=600, unit="cm",  file=paste0("effects on change in diversity across scales at year ", ey, ".png"))
}
...
for(i in unique(summary.intercepts.across.sites$combi)){
  ...
  df.ticks <- data.frame(x = seq(-3, 3, by = 0.1))%>%mutate(x.position= x + 0.06)
...
# combine plots 
####################### for sites with more than 10 years 
# for native and non-native with hill of 0
(pp.div1<-plot_grid(list_plots$NAT_10_0 ,
                    list_plots$INT_10_0,
                    nrow=1, ncol=2, 
                    vjust = -0.01, hjust = -0.02,
                    labels = c("A (10-year nutrient addition; native species)", "B (10-year nutrient addition; non-native species)"), label_fontface = "bold", label_size = 11))
...
ggsave(pp.div3, width = 20, height = 10, dpi=600, unit="cm",  file="effects on change in diversity across scales for native and non-native for sites with more than 10 years data.png")

# for life forms with hill of 0
(pp.div1<-plot_grid(list_plots$FORB_10_0,
                    list_plots$GRAMINOID_10_0,
                    list_plots$LEGUME_10_0,
                    list_plots$WOODY_10_0,
                    nrow=2, ncol=2, 
                    vjust = -0.01, hjust = -0.02, 
                    labels = c("A (10-year nutrient addition; forb)", "B (10-year nutrient addition; graminoid)", "C (10-year nutrient addition; legume)", "D (10-year nutrient addition; woody)"), label_fontface = "bold", label_size = 11))
(pp.div3<-pp.div1+ theme(plot.margin = margin(l = 10, b = 10, t=15))+
    draw_label("Effects of nutrient addition on alpha diversity", x = 0.5, y = 0, size = 11) + draw_label("Effects of nutrient addition on gamma diversity", x = 0, y = 0.5, angle = 90, size = 11))
ggsave(pp.div3, width = 20, height = 20, dpi=600, unit="cm",  file="effects on change in diversity across scales for life forms for sites with more than 10 years data.png")

####################### for sites with more than 14 years 
# for native and non-native with hill of 0
(pp.div1<-plot_grid(list_plots$NAT_14_0 ,
                    list_plots$INT_14_0,
                    nrow=1, ncol=2, 
                    vjust = -0.01, hjust = -0.02, 
                    labels = c("A (14-year nutrient addition; native species)", "B (14-year nutrient addition; non-native species)"), label_fontface = "bold", label_size = 11))
(pp.div3<-pp.div1+ theme(plot.margin = margin(l = 10, b = 10, t=15))+
    draw_label("Effects of nutrient addition on alpha diversity", x = 0.5, y = 0, size = 11) + draw_label("Effects of nutrient addition on gamma diversity", x = 0, y = 0.5, angle = 90, size = 11))
ggsave(pp.div3, width = 20, height = 10, dpi=600, unit="cm",  file="effects on change in diversity across scales for native and non-native for sites with more than 14 years data.png")

# for life forms with hill of 0
(pp.div1<-plot_grid(list_plots$FORB_14_0,
                    list_plots$GRAMINOID_14_0,
                    list_plots$LEGUME_14_0,
                    list_plots$WOODY_14_0,
                    nrow=2, ncol=2, 
                    vjust = -0.01, hjust = -0.02, 
                    labels = c("A (14-year nutrient addition; forb)", "B (14-year nutrient addition; graminoid)", "C (14-year nutrient addition; legume)", "D (14-year nutrient addition; woody)"), label_fontface = "bold", label_size = 11))
(pp.div3<-pp.div1+ theme(plot.margin = margin(l = 10, b = 10, t=15))+
    draw_label("Effects of nutrient addition on alpha diversity", x = 0.5, y = 0, size = 11) + draw_label("Effects of nutrient addition on gamma diversity", x = 0, y = 0.5, angle = 90, size = 11))
ggsave(pp.div3, width = 20, height = 20, dpi=600, unit="cm",  file="effects on change in diversity across scales for life forms for sites with more than 14 years data.png")


###########################################################################################
### relationships between change in  diversity and site biotic and abiotic factors ####
###########################################################################################
#all.species.long.term<-read.csv("estimated alpha, gamma, and beta diversity for at individual sites with longer term.csv")
all.species.long.term<-categorize.beta
...
year.used.for.climate<-d7%>%filter(year_trt==yr)%>%select(site_code, year, year_trt)%>%distinct()
# sites and blocks used in richness data 
site.block.select<-d7%>%filter(year_trt==yr)%>%mutate(site.block=paste(site_code, block, sep="_"))%>%select(site.block)%>%distinct()


summary.bivariate.estimate.coef.long.term<-c()
for (yr in c(10, 14)){
  # yr<-14
  drought1<-drought%>%merge(year.used.for.climate, by=c("site_code"))%>%filter(year.x <= year.y & year.x >= (year.y- yr) )%>%
  ...
  site.productivity1<-site.productivity%>%mutate(site.block=paste(site_code, block, sep="_"))%>%
    filter(site.block %in% site.block.select$site.block)%>% filter(trt %in% c("Control"))%>%filter(year_trt<=yr)
  ...
  biomass.removed.by.herbivore1<-biomass.removed.by.herbivore%>%mutate(site.block=paste(site_code, block, sep="_"))%>%
    filter(site.block %in% site.block.select$site.block)%>% filter(year_trt<=yr)
  # check years included for each site 
  ...
  table(check.year.biomass.removed$N.years)# 
  ...
  # it is very important that each site should have the same number of years. Sampling more times may find more species. For sites having data at year_trt 14, they do not 
  # always have continuous data collection
  # so I will use 10-year data only
  site.species.pool1<-site.species.pool%>%filter(site_code %in% year.used.for.climate$site_code)%>% filter(year_trt<=10)
  # check years included for each site 
  ...
  table(check.year.richness$N.years)# majority of the sites have ur+1 years' data 
  # it is very important that each site should have the same number of years. Sampling more times may find more species 
  check.year.richness.yr.years<-check.year.richness%>%filter(N.years < 11)
  # for those sites that only sample for 9 years, add year  yr+1 data if possible 
  site.species.pool_sub.sites<-site.species.pool%>%filter(site_code %in% check.year.richness.yr.years$site_code)%>% filter(year_trt<= 11)
  check.year.richness_sub<-site.species.pool_sub.sites%>%ungroup()%>%select(site_code, year_trt)%>%distinct()%>%group_by(site_code)%>%summarise(N.years=length(year_trt))%>% filter(N.years== 11)
  
  site.species.pool2<-site.species.pool1%>%filter(!site_code %in%check.year.richness.yr.years$site_code)%>%
    bind_rows(site.species.pool1%>%filter(site_code %in%check.year.richness_sub$site_code)%>% filter(year_trt<= 11))%>%
    group_by(site_code)%>%summarise(site.species.pool=mean(HillDiv))
  
  change.in.div.cov<-all.species.long.term%>%filter(q==0 & functional_group=="all" & experimental.year==yr)%>%select(experimental.year, site_code, functional_group, true.alpha_mean, true.gamma_mean, change.beta_mean)%>%
    pivot_longer(cols = c("change.beta_mean",  "true.alpha_mean",   "true.gamma_mean"), names_to = "diversity.facet", values_to = "diversity.value")%>%
  ...
  cat.col<-all.species.long.term%>%filter(q==0 & functional_group=="all" & experimental.year==yr)%>%select(site_code, cat.process)
  ...
  ggsave(pp.relation.raw, width = 21, height=14.9, unit="cm", file=paste0("relationships between change in diversity for all species for sites with ", yr, "-year nutrient addition and environmental factors.PNG"))
  ...
  summary.bivariate.estimate.coef1<-summary.bivariate.estimate.coef%>%mutate_at(vars(c(1:7)), round, digits=3)%>%
    ...
    merge(change.in.div.all_l%>%select(diversity.facet1, name1, combi.id)%>%distinct(), by=c("combi.id"))%>%filter(!name1 %in% c("Herbivory"))
  # relevel 
  ...
  (pp.relation.pred_1 <- change.in.div.all_l%>%filter(!name1 %in% c("Herbivory"))%>%merge(cat.col, by = c("site_code")) %>%
  ...
  ggsave(pp.relation.pred_1, width = 20, height=20, unit="cm", file=paste0("predicted bivariate relationships between change in diversity for all species for sites with ", yr, "-year nutrient addition and environmental factors.PNG"))
  
  summary.bivariate.estimate.coef.long.term<-summary.bivariate.estimate.coef.long.term%>%bind_rows(summary.bivariate.estimate.coef1%>%mutate(experimental.year=yr))
}
