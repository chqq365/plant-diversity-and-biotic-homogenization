...
for(vi in unique(variable.id.combi$variable.id)){
  # vi<-"alldif.gamma40"
  load(paste0("model for diversity change for combination of ",vi, " for sites with four blocks.Rdata"))
  #plot(mod.eff.nut)
...
# write.csv(summary.model.fit1, file="estimated overall effects and model fit for sites with four blocks.csv")
...
(ppp2<-ppp1+
    ...
    geom_point(data=eff.nut.overall.yr,  aes(x=Estimate_alpha, y=Estimate_gamma), size=5, stroke = 1, pch=21, alpha=0.7, color="black")+ labs(title = paste0(nrow(eff.nut.rich.yr), " sites with four blocks")))
ggsave(ppp2, width = 15, height = 15, dpi=600, unit="cm",  file=paste0("change in species richness across scales at year ",yr, " for sites with four blocks.png"))
...
ggsave(pp.life.forms3, width = 20, height = 20, dpi=600, unit="cm",  file=paste0("effects on change in diversity across scales for life forms at year ",yr, " for sites with four blocks.png"))
...
(pp.origins3<-pp.origins1+ theme(plot.margin = margin(l = 10, b = 10, t=15))+
    draw_label( expression(bar("∆α") ~ " (LRR)"), x = 0.5, y = 0, size = 11) + draw_label("∆γ (LRR)", x = 0, y = 0.5, angle = 90, size = 11))
ggsave(pp.origins3, width = 20, height = 10, dpi=600, unit="cm",  file=paste0("effects on change in diversity across scales for native and non-native species at year ",yr, " for sites with four blocks.png"))
