...
regions1$Scenarios<-factor(regions1$Scenarios, levels=c("Gain of widespread species", "Spatially restricted replaced by widespread species", "Loss of spatially restricted species",
                                                        "Gain of spatially restricted species",  "Widespread replaced by spatially restricted species", "Loss of widespread species"))
...
pp<-ggplot() +theme_cowplot(font_size = 11)+  # Apply cowplot theme
  ...
  annotate(geom = "text", x = min_value/1.5, y = max_value/1.5, angle = 45, label = "Differentiation", color="white", size =5, fontface = "bold") +  
  annotate(geom = "text", x = min_value/1.65, y = max_value/1.65, angle = 45, label = "Increase in ∆β (LRR)", color="white", size =4) +  
  annotate(geom = "text", x = max_value/1.5, y = min_value/1.5, angle = 45, label = "Homogenization", color="white", size =5, fontface = "bold") +
  annotate(geom = "text", x = max_value/1.35, y = min_value/1.35, angle = 45, label = "Decrease in ∆β (LRR)", color="white", size =4) +
  annotate("label", x = 0, y = 0, angle = 45, label = "No change in beta diversity; ∆β (LRR)", fill="white", size =5, fontface = "bold") +
  
  annotate(geom = "text", x = 0.40*max_value, y=0.2*max_value, angle = 0, label = "I", size =4) +
  annotate(geom = "text",  x = 0.10*max_value, y =  0.2*min_value, angle = 0, label = "II", size =4) +
  annotate(geom = "text", x = 0.6*min_value, y = 0.80*min_value, angle = 0, label = "III", size =4) +
  annotate(geom = "text", x = 0.9*min_value, y = 0.2*min_value, angle = 0, label = "IV", size =4) +
  annotate(geom = "text", x =0.9*min_value, y = 0.2*max_value, angle = 0, label = "V", size =4) +
  annotate(geom = "text", x = 0.1*max_value,  y = 0.8*max_value,  angle = 0, label = "VI", size =4)+
  
  annotate(geom = "text", x = 0.60*max_value, y=0.2*max_value, angle = 0, label = "Gain of\n widespread\n species", size =4) +
  annotate(geom = "text",  x = 0.55*max_value, y =  0.2*min_value, angle = 0, label = "Spatially restricted replaced \n by widespread species", size =4) +
  annotate(geom = "text", x = 0.3*min_value, y = 0.80*min_value, angle = 0, label = "Loss of spatially \nrestricted species", size =4) +
  annotate(geom = "text", x = 0.7*min_value, y = 0.2*min_value, angle = 0, label = "Loss of\n widespread\n species", size =4) +
  annotate(geom = "text", x =0.5*min_value, y = 0.2*max_value, angle = 0, label = "Widespread replaced by\n spatially restricted species", size =4) +
...
ggsave(pp1, width = 15, height = 15, dpi=600, unit="cm",  file="conceptual figure.png")

leg<-get_legend(pp)
(pp.leg<-plot_grid(leg))
ggsave(pp.leg, width = 16, height = 2, dpi=600, unit="cm",  file="legend.png")
