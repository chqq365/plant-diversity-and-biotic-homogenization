...
eff.nut.alpha<-alpha.gamma%>%filter(scale %in% c("alpha"))%>%group_by(site_code, trt, year_trt, functional_group, q, scale, block)%>%summarise(HillDiv1=mean(HillDiv))%>%
  ...
  filter(!is.na(Control))%>%filter(!is.na(NPK)) %>% mutate(dif.alpha=log(NPK/Control))%>%mutate(scale="dif.alpha", dif.value=dif.alpha)%>%select(site_code, block, year_trt, functional_group, q, scale, dif.value)%>%distinct()
...
eff.nut.alpha.beta.gamma<-eff.nut.beta%>% filter(!is.na(alpha))%>%filter(!is.na(dif.gamma))%>%select(site_code, year_trt, functional_group, q, dif.gamma, dif.beta)%>%
  pivot_longer(cols = c("dif.gamma", "dif.beta"), names_to = "scale", values_to = "dif.value")%>%mutate(block=0)%>%select(site_code, block, year_trt, functional_group, q, scale, dif.value)%>%
...
#eff.nut.alpha.beta.gamma1<-eff.nut.alpha.beta.gamma%>%filter(q==0)%>%filter(functional_group=="all")
...
###########################################################################################
# effects of nutrient addition on diversity
...
alpha.gamma0<-calculate.alpha.gamma.diversity(data.more.blocks)
alpha.gamma<-alpha.gamma0%>%filter(q==0 )
eff.nut.alpha<-alpha.gamma%>%filter(scale %in% c("alpha"))%>%group_by(site_code, trt, year_trt, functional_group, q, scale, block)%>%summarise(HillDiv1=mean(HillDiv))%>%
  pivot_wider(names_from = "trt", values_from = "HillDiv1")%>%
  filter(!is.na(Control))%>%filter(!is.na(NPK)) %>% mutate(dif.alpha=log(NPK/Control))%>%mutate(scale="dif.alpha", dif.value=dif.alpha)%>%select(site_code, block, year_trt, functional_group, q, scale, dif.value)

eff.nut.beta<-alpha.gamma%>%group_by(site_code, trt, year_trt, functional_group, q, scale, block)%>%summarise(HillDiv1=mean(HillDiv))%>%
  pivot_wider(names_from = "trt", values_from = "HillDiv1")%>%
  filter(!is.na(Control))%>%filter(!is.na(NPK)) %>% mutate(dif.HillDiv=log(NPK/Control))%>%
  group_by(site_code, year_trt, functional_group, q, scale)%>%summarise(dif.HillDiv1=mean(dif.HillDiv))%>%pivot_wider(names_from = "scale", values_from = "dif.HillDiv1")%>%mutate(dif.beta=gamma - alpha)%>%rename(dif.gamma=gamma)
eff.nut.alpha.beta.gamma1<-eff.nut.beta%>% filter(!is.na(alpha))%>%filter(!is.na(dif.gamma))%>%select(site_code, year_trt, functional_group, q, dif.gamma, dif.beta)%>%
  pivot_longer(cols = c("dif.gamma", "dif.beta"), names_to = "scale", values_to = "dif.value")%>%mutate(block=0)%>%select(site_code, block, year_trt, functional_group, q, scale, dif.value)%>%
  bind_rows(eff.nut.alpha)%>% mutate(variable.id=paste0(functional_group, scale, year_trt, q))
unique(eff.nut.alpha.beta.gamma1$variable.id)
