...
cover.data<-read.csv(file="cover data for control and NPK at 72 NutNet sites.csv")
alpha.gamma<-calculate.alpha.gamma.diversity(cover.data)
...
eff.nut$cat.process<-factor(eff.nut$cat.process, levels = c("Gain of widespread species", "Spatially restricted replaced by widespread species", "Loss of spatially restricted species",
                                                            ...
                                                            "Other situations"))
# save the data 
write.csv(eff.nut, file = "effects of nutrient addition on diversity across scales.csv")
...
(process.group<-table(check.year.4$cat.process)%>%data.frame()%>%mutate(`Groups by ∆β` = rep(c("Homogenization", "Differentiation", " "), times=c(3,3,3))))
...
summary.diversity.individual.sites1.yr4<- summary.diversity.individual.sites%>%merge(variable.id.combi, by=c("variable.id"))  %>%
  ...
  mutate(cat.process.ci=case_when((Intercept_dif.alpha>Intercept_dif.gamma & Intercept_dif.alpha>0 &Intercept_dif.gamma>0 & ((Intercept_lower_dif.alpha > 0 & Intercept_upper_dif.alpha > 0) & (Intercept_lower_dif.gamma > 0 & Intercept_upper_dif.gamma > 0)))~"Gain of widespread species",
                                  (Intercept_dif.alpha>0 &Intercept_dif.gamma<0 & ((Intercept_lower_dif.alpha > 0 & Intercept_upper_dif.alpha > 0) & (Intercept_lower_dif.gamma < 0 & Intercept_upper_dif.gamma < 0)))~"Spatially restricted replaced by widespread species",
                                  ...
                                  (Intercept_dif.alpha<0 &Intercept_dif.gamma>0 & ((Intercept_lower_dif.alpha < 0 & Intercept_upper_dif.alpha < 0) & (Intercept_lower_dif.gamma > 0 & Intercept_upper_dif.gamma > 0)))~"Widespread replaced by spatially restricted species",
  ...
  mutate(cat.process=case_when((Intercept_dif.alpha>Intercept_dif.gamma & Intercept_dif.alpha>0 &Intercept_dif.gamma>0 )~"Gain of widespread species",
                               (Intercept_dif.alpha>0 &Intercept_dif.gamma<0 )~"Spatially restricted replaced by widespread species",
                               ...
                               (Intercept_dif.alpha<0 &Intercept_dif.gamma>0)~"Widespread replaced by spatially restricted species",
...
##########################################################################################
###### change in species richness across scales in year 4 and 14  
###########################################################################################
...
for(yr in c(4, 14)){ 
  ...
  (ppp2<-ppp1+
      ...
      geom_point(data=eff.nut.overall.yr,  aes(x=Estimate_alpha, y=Estimate_gamma), size=6, stroke = 1, pch=21, alpha=0.7, color="black"))
if(yr==4){
  ppp3<-ppp2
}else{
  (ppp3<-ppp2+labs(title = paste0(nrow(eff.nut.rich.yr), " sites with ", yr, " years after nutrient addition began")))
}
  leg.space_1<-plot_grid(spacer, leg, nrow = 1, rel_widths = c(1, 99)) # from the file function to draw background for main figures.R
  (ppp4 <-plot_grid(ppp3 + theme(plot.margin = margin(0, 5, 5, 5)) , 
                    leg.space_1, 
                   nrow=2,  align="v", rel_heights = c(8, 1))) 
 ggsave(ppp4, width = 18, height = 19, dpi=600, unit="cm",  file=paste0("change in species richness across scales at year ",yr, ".png"))
 ggsave(ppp4, width = 18, height = 19, unit="cm",  file=paste0("change in species richness across scales at year ",yr, ".pdf"), device = cairo_pdf)
...
for( vi in unique(variable.id.combi_yr4_all.species$variable.id)){
  ...
  load(paste0("model for diversity change for combination of ",vi, ".Rdata") )   
  # check model fitting 
  check.pred<-pp_check(mod.eff.nut)+ theme_cowplot(font_size = 11)+panel_border()+
...
ggsave(check.fit1, width = 18, height=10, unit="cm",  file=paste0("check model fitting for change in species richness across scales in year 4.png"))

##########################################################################################
###### change in species richness for species groups across scales in year 4 and 14 
###########################################################################################

list_plots <- vector('list', 15)
for(yr in c(4, 14)){ 
  ...
  ggsave(pp.life.forms4, width = 18, height = 20, dpi=600, unit="cm",  file=paste0("effects on change in diversity across scales for life forms at year ",yr, ".png"))
  ggsave(pp.life.forms4, width = 18, height = 20,  unit="cm",  file=paste0("effects on change in diversity across scales for life forms at year ",yr, ".pdf"), device = cairo_pdf)
  ...
  (pp.origins4 <-plot_grid(pp.origins3 + theme(plot.margin = margin(1, 5, 5, 5)) , 
                           leg.space, 
                           nrow=2, align = "v",  rel_heights = c(8, 1.7))) 
  ggsave(pp.origins4, width = 18, height = 10, dpi=600, unit="cm",  file=paste0("effects on change in diversity across scales for native and non-native species at year ",yr, ".png"))
  ggsave(pp.origins4, width = 18, height = 10, unit="cm",  file=paste0("effects on change in diversity across scales for native and non-native species at year ",yr, ".pdf"), device = cairo_pdf)
...
(pp.div1<-plot_grid(list_plots1$hill1,
                    ...
                    # vjust = -0.01,
                    hjust=-0.25,
                    labels = c("A (ENS based on Shannon diversity)", "B (ENS based on Simpson diversity)"), label_fontface = "bold", label_size = 11))
(pp.div3<-pp.div1+ theme(plot.margin = margin(l = 10, b = 10, t=15))+
    draw_label(expression(bar("∆α") ~ " (LRR)"), x = 0.5, y = 0, size = 11) + draw_label("∆γ (LRR)", x = 0, y = 0.5, angle = 90, size = 11))
(pp.div4 <-plot_grid(pp.div3 + theme(plot.margin = margin(1, 5, 5, 5)) , 
                         leg.space, 
                         nrow=2, align = "v",  rel_heights = c(8, 2))) 
ggsave(pp.div4, width = 18, height = 10, dpi=600, unit="cm",  file=paste0("effects on change in Shannon diversity and Simpson diversity at year ",yr, ".png"))
...
Fig2_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("All species"))%>%filter(year_trt%in% c(4))
Fig3_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("Native",  "Non-native"))%>%filter(year_trt%in% c(4))
Fig4_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("FORB", "GRAMINOID", "LEGUME", "WOODY"))%>%filter(year_trt%in% c(4))
SFig4_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Shannon diversity", "Simpson diversity"))%>%filter(`Species groups` %in% c("All species"))%>%filter(year_trt%in% c(4))

SFig5_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("All species"))%>%filter(year_trt%in% c(14))
SFig6_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("Native",  "Non-native"))%>%filter(year_trt%in% c(14))
SFig7_data_source<-sites.and.estimate%>%filter(`Diversity metrics` %in% c("Species richness"))%>%filter(`Species groups` %in% c("FORB", "GRAMINOID", "LEGUME", "WOODY"))%>%filter(year_trt%in% c(14))

write.csv(Fig2_data_source, file="Fig2_data_source.csv")
write.csv(Fig3_data_source, file="Fig3_data_source.csv")
write.csv(Fig4_data_source, file="Fig4_data_source.csv")

write.csv(SFig4_data_source, file="SFig4_data_source.csv")
write.csv(SFig5_data_source, file="SFig5_data_source.csv")
write.csv(SFig6_data_source, file="SFig6_data_source.csv")
write.csv(SFig7_data_source, file="SFig7_data_source.csv")
